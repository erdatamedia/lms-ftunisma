...
export function AttendanceManager({ meetingId }: AttendanceManagerProps) {
  ...
  return (
    <div className="space-y-6">
      <h5 className="text-xs font-bold uppercase tracking-wider text-slate-400">Buka Sesi Absensi</h5>
            ...
            <div
              ...
              className={`rounded-2xl border transition p-4 ${
                activeView === type
                  ...
                  : 'border-slate-200/50 bg-slate-50/50 dark:bg-slate-800/20'
              }`}
            >
              <div className="mb-3 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Absensi {type}</span>
                <SessionBadge active={isOpened} />
              </div>
              <button
                onClick={() => openSession(type)}
                disabled={loadingType !== null}
                className="w-full rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100 disabled:opacity-60"
              >
                {isLoading
                  ? `Membuka ${type}...`
                  : isOpened
              ...
              {isOpened && (
                <button
                  ...
                  className="mt-2 w-full rounded-xl border border-slate-200/50 py-2 text-xs font-bold text-slate-600 transition hover:bg-slate-100 dark:hover:bg-slate-800"
                >
